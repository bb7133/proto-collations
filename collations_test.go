...
func unicodeWeight(r rune) uint32 {
	plane := plane00
	if (r >> 8) > 0 {
		plane = plane01
	}
	return plane[r&0xFF][2]
}

func weightString(str string) string {
	buffer := bytes.NewBuffer(make([]byte, len(str)))
	for len(str) > 0 {
		r, size := utf8.DecodeRuneInString(str)
		if r == utf8.RuneError {
			return ""
		}
		binary.Write(buffer, binary.BigEndian, unicodeWeight(r))
		str = str[size:]
	}
	return buffer.String()
}

func weightCompare(s1, s2 string) int {
	for len(s1) > 0 && len(s2) > 0 {
		r1, size := utf8.DecodeRuneInString(s1)
		if r1 == utf8.RuneError {
			return -1
		}
		r2, size := utf8.DecodeRuneInString(s1)
		if r2 == utf8.RuneError {
			return 1
		}
		cmp := int(unicodeWeight(r1) - unicodeWeight(r2))
		if cmp != 0 {
			return cmp
		}
		s1 = s1[size:]
...
func BenchmarkGoCollations(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		col.CompareString(s1, s2)
	}
}

func BenchmarkWeight(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = weightCompare(s1, s2)
	}
}
